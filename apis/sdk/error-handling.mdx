...
The Promptrun SDK provides comprehensive error handling through custom error classes, enabling you to build resilient applications that gracefully handle API issues, network problems, authentication failures, and other edge cases.
  ...
  const pollingPrompt = await promptrun.prompt({
    ...
    onPollingError: (pollingError) => {
      ...
      switch (pollingError.type) {
        case "rate_limit":
          console.log("⏳ Rate limited - automatic backoff applied");
          console.log(
      ...
      if (pollingError.consecutiveErrors >= 5) {
        console.error("🚨 ALERT: Multiple consecutive polling errors detected");
        console.error(
    ...
    try {
      this.pollingPrompt = await promptrun.prompt({
        ...
        onPollingError: (error) => {
          this.errorCount++;
          console.error(`❌ Polling error #${this.errorCount}:`, error.message);

          if (this.errorCount >= 10) {
            console.error("🔴 Circuit breaker activated - stopping polling");
            this.pollingPrompt.stopPolling();
            this.scheduleReconnection();
          }

          if (this.errorCount === 3) {
            console.warn("⚠️ Warning: 3 consecutive polling errors");
          } else if (this.errorCount === 5) {
            console.error(
              "🚨 Alert: 5 consecutive polling errors - investigation needed"
            );
          }
        },
      });
      ...
      console.log("✅ Resilient prompt service initialized");
    } catch (error) {
      console.error("❌ Failed to initialize prompt service:", error.message);
      console.log("🔄 Using fallback prompt");
    }
  }
...
class ProductionChatService {
  ...
  async initialize() {
    try {
      ...
      console.log("✅ Production chat service initialized successfully");
    } catch (error) {
  ...
  private handleInitializationError(error: any) {
    ...
    if (error instanceof PromptrunAuthenticationError) {
      ...
      console.error(
        "🔑 Authentication Error during initialization:",
        error.message
      );
    } else if (error instanceof PromptrunAPIError) {
      this.errorStats.api++;
  ...
  private sendAlert(severity: string, message: string) {
    console.error(`🚨 ${severity.toUpperCase()} ALERT: ${message}`);
  }
  ...
  shutdown() {
    console.log("🔄 Shutting down production chat service...");
    if (this.pollingPrompt) {
      this.pollingPrompt.stopPolling();
    }

    console.log("📊 Final error statistics:", this.getErrorStats());
    console.log("✅ Production chat service shutdown complete");
  }
}
